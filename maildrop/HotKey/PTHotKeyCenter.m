...
- (id)init
{
	...
	if( self )
	{
		...
		mHotKeysByHotKeyIdIds = [[NSMutableDictionary alloc] init];
	}
...
- (BOOL)registerHotKey: (PTHotKey*)hotKey
{
	...
	hotKeyID.id = 1;//keith says: this only works because we only have one.  This should be some kind of counter?
	...
	if( hotKey && key ) {
		...
		[mHotKeysByHotKeyIdIds setObject:hotKey forKey:[NSNumber numberWithInteger:hotKeyID.id]];
	}
...
//	NSAssert( hotKeyID.id != nil, @"Invalid hot key id" ); //keith says: can't compare pointer to int anymore

	hotKey = (PTHotKey*)[mHotKeysByHotKeyIdIds objectForKey: [NSNumber numberWithInteger: hotKeyID.id]];
